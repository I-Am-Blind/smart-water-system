...
| | |
|---|---|
| Endpoint | WebSocket, path `/ws` on the server (laptop: `ws://192.168.0.3:3000/ws`; cloud: `wss://<host>/ws`) |
...
- Sensor arrays `f` and `p` have 7 entries in this order: `[m, b1i, b1o, b2i, b2o, b3i, b3o]` = master, branch1 IN, branch1 OUT, branch2 IN, branch2 OUT, branch3 IN, branch3 OUT.
- Branch arrays `loss`, `leak`, `v` have 3 entries, index 0..2 = branch 1..3.
- A scalar branch field `b` is 1, 2 or 3.
...
```json
{"t":"hello","proto":1,"id":"rig-7a3f21","fw":"1.0.0","ip":"192.168.0.42","rssi":-58,"rst":"POWERON","sim":false}
```
`id` = `"rig-"` + last 6 hex digits of the MAC (`"sim"` for the fake device). `rst` = reset reason text (POWERON, SW, PANIC, INT_WDT, TASK_WDT, WDT, DEEPSLEEP, BROWNOUT, SDIO, UNKNOWN).

### tel (1 Hz, latest snapshot only, never a backlog)
```json
{"t":"tel","ms":123456,"seq":120,
 "f":[3.61,1.22,1.20,1.21,0.58,1.18,1.17],
 "p":[3541234,1200011,1180002,1199870,600120,1170022,1160500],
 "loss":[1.6,52.1,0.8],"leak":[0,3,0],"mleak":0,"v":[1,0,1],"pump":1,
...
| `f` | number[7] | L/min, 2 dp |
| `p` | uint32[7] | cumulative raw pulses (uncalibrated) |
| `loss` | number[3] | `(in-out)/in*100` over the 3 s window, 1 dp, `0` when the check is gated (valve closed, settling, IN < 0.5 L/min) |
| `leak` | int[3] | 0 ok, 1 warn, 2 drip (latched, valve closed), 3 burst (latched, valve closed) |
| `mleak` | 0/1 | manifold leak latched (master exceeds sum of open-branch inflows) |
| `v` | 0/1[3] | valve relay state, 1 = open (relay energised) |
...
### evt (on state change)
```json
{"t":"evt","ms":123456,"ev":"leak","b":2,"kind":"burst","loss":62.5,"src":"leak"}
{"t":"evt","ms":123999,"ev":"valve","b":2,"on":0,"src":"leak"}
{"t":"evt","ms":130000,"ev":"pump","on":0,"src":"wd","reason":"max_on"}
{"t":"evt","ms":140000,"ev":"mleak","loss":30.1,"src":"leak","reason":"manifold"}
{"t":"evt","ms":150000,"ev":"leak_clear","src":"serial"}
{"t":"evt","ms":160000,"ev":"all_off","src":"ws"}
{"t":"evt","ms":40,"ev":"boot","src":"boot"}
{"t":"evt","ms":170000,"ev":"sim","on":1,"src":"serial"}
```
`ev` ∈ `boot | leak | leak_clear | mleak | valve | pump | all_off | sim`. `src` ∈ `serial | ws | http | leak | wd | interlock | boot` (who caused it). `reason` ∈ `max_on | all_closed | manifold` (why the watchdog/interlock/leak logic acted). `kind` ∈ `drip | burst`. `on` is the NEW state for valve / pump / sim.

### ack (reply to every cmd, within 3 s)
```json
{"t":"ack","id":17,"ok":true}
{"t":"ack","id":17,"ok":false,"err":"latched"}
{"t":"ack","id":21,"ok":true,"ms":123500}
```
`err` ∈ `latched` (branch leak latched, valve on refused until reset_leak) | `bad_branch` | `no_open_valve` (pump refused, all valves closed) | `unknown_act` | `bad_json`. Successful acks carry the device `ms` (always present for `ping`).

## 3. Server → device

### cmd
```json
{"t":"cmd","id":17,"act":"valve","b":2,"on":true,"dur":120}
{"t":"cmd","id":18,"act":"pump","on":true,"dur":120}
{"t":"cmd","id":19,"act":"all_off"}
{"t":"cmd","id":20,"act":"reset_leak"}
{"t":"cmd","id":21,"act":"ping"}
{"t":"cmd","id":22,"act":"sim","on":true,"b":2,"pct":40}
```
`id` uint32 assigned by the server, echoed in the ack. `act` ∈ `valve | pump | all_off | reset_leak | ping | sim`. `b` required for valve (1..3). `dur` seconds the relay may stay on; absent/0 = firmware default (valve 600 s cap, pump 20 s default / 300 s cap). `sim`: `on:true` enables simulation and, when `b`+`pct` are given, injects a `pct` % loss on branch `b`; `on:false` without `b` disables simulation and clears injected leaks.
...
### cmd
```json
{"t":"cmd","cid":"v-1725900000-1","act":"valve","b":2,"on":false}
...
{"t":"state","now":1725900001000,"online":true,"lastSeen":1725900000900,
 ...
 "brand":{"name":"Cascade","shortName":"Cascade","tagline":"...","deviceName":"rig","team":"","school":"","branches":["Branch 1","Branch 2","Branch 3"],"colors":{"bg":"#0a1224","accent":"#22d3ee","ok":"#34d399","warn":"#fbbf24","danger":"#f43f5e"},"showQr":true},
 "serverUrl":"http://192.168.0.3:3000"}
```
...
```json
{"t":"err","msg":"invalid tel: f must have 7 items"}
```
...
| `GET /api/history?minutes=30` | columnar `{step, t[], m[], b1i[], b1o[], b2i[], b2o[], b3i[], b3o[], loss1[], loss2[], loss3[], leak[], ntu[], ppm[]}`; minutes clamped 1..1440; ≤ 600 buckets (`step` ms); `leak` is a bitmask per bucket, bit0 = branch 1, max over the bucket |
...
| `POST /api/cmd` body `{act,b?,on?,dur?,pct?}` | `{ok, err?}`; HTTP 503 when device offline, 504 on ack timeout, 400 on bad body |

Device-local HTTP (firmware, port 80, `http://rig.local`): `GET /status` → the current `tel` JSON; `POST /cmd` with a cmd body (no `t`/`id` needed) → `{ok, err?}` with HTTP 200 on success, 409 when the device refuses (`latched`, `no_open_valve`, `bad_branch`), 400 on a bad body, 504 if the control loop did not answer within 500 ms. This is a fallback for when the laptop server is down.
...
4. Leak latched (level 2/3): the branch valve is closed by the device (`src:"leak"`) and `valve on` for that branch is refused with `latched` until `reset_leak`. `reset_leak` clears latches only; it does not reopen valves or restart the pump.
5. Manifold leak (`mleak`): device stops the pump. Cleared by `reset_leak`.
6. The device keeps detecting leaks and enforcing all of the above with no Wi-Fi and no server.
7. Telemetry is 1 Hz. The server tolerates 0.5–2 Hz. Only the latest snapshot is ever sent; nothing is queued while offline.
