...
const branch = z.union([z.literal(1), z.literal(2), z.literal(3)]);
...
export const BrandSchema = z.object({
  ...
  branches: z.tuple([z.string().min(1).max(24), z.string().min(1).max(24), z.string().min(1).max(24)]),
  colors: z.object({
...
export const TelemetrySchema = z.object({
  ...
  f: z.array(z.number().nonnegative()).length(7),
  p: z.array(z.number().int().nonnegative()).length(7),
  loss: z.array(z.number()).length(3),
  leak: z.array(leakLevel).length(3),
  mleak: onOff,
  v: z.array(onOff).length(3),
  pump: onOff,
...
export const RigEventSchema = z.object({
  t: z.literal("evt"),
  ms: z.number().int().nonnegative(),
  ev: z.enum(["boot", "leak", "leak_clear", "mleak", "valve", "pump", "all_off", "sim"]),
  b: branch.optional(),
  kind: z.enum(["drip", "burst"]).optional(),
  loss: z.number().optional(),
  on: onOff.optional(),
  src: z.enum(["serial", "ws", "http", "leak", "wd", "interlock", "boot"]),
  reason: z.enum(["max_on", "all_closed", "manifold"]).optional(),
});
...
export const CmdBodySchema = z.object({
  act: z.enum(["valve", "pump", "all_off", "reset_leak", "ping", "sim"]),
  b: branch.optional(),
